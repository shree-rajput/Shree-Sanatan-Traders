...
const OrderSuccess = () => {
  ...
  const steps = [
    { label: "Order Placed", done: true, icon: <ClockFading size={18} /> },
    { label: "Confirmed", done: false, icon: <Package size={18} /> },
    { label: "Packed", done: false, icon: <PackageCheck size={18} /> },
    { label: "Shipped", done: false, icon: <Truck size={18} /> },
    { label: "Delivered", done: false, icon: <House size={18} /> },
  ];
      ...
      <div className="max-w-lg w-full space-y-6">
        ...
        <div className="bg-white rounded-3xl shadow-2xl shadow-green-100 border border-green-100 p-10 text-center">
          ...
          <p className="text-gray-500 text-sm mb-1">
            Your order is SucessFully Placed, Thank you for trusting us.
          </p>
            ...
            {steps.map((step, idx) => (
              <React.Fragment key={idx}>
                <div className="flex flex-col items-center gap-1.5">
                  ...
                  <span
                    className={`text-[9px] font-bold text-center ${step.done ? "text-green-600" : "text-gray-400"}`}
                  >
                    {step.label}
                  </span>
                </div>
                {idx < steps.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-1 ${idx === 0 ? "bg-green-500" : "bg-gray-200"}`}
                  />
                )}
              </React.Fragment>
            ))}
