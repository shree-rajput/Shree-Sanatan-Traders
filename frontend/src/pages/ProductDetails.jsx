...
import { LuShoppingCart, LuChevronLeft, LuCheck, LuArrowRight, LuMinus, LuPlus, LuTriangleAlert } from "react-icons/lu";
    ...
    <div className="min-h-screen bg-white py-12 md:py-20 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          {/* Image */}
          <div className={`relative bg-gray-50 rounded-[40px] p-12 md:p-20 border border-gray-100 aspect-square flex items-center justify-center overflow-hidden ${isOutOfStock ? "opacity-60" : ""}`}>
            {isOutOfStock && (
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-400 font-medium">{t("inclusive_taxes")}</p>
            </div>

            {product.description && (
              <div>
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-3">{t("product_description")}</h3>
            ...
            <div className="pt-8 border-t border-gray-100 space-y-6">
              ...
              {isOutOfStock ? (
                <div className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-2xl px-6 py-4">
                  ...
                  <div>
                    <p className="font-bold text-red-700">{t("out_of_stock")}</p>
                    <p className="text-sm text-red-500">स्टॉक समाप्त — This item is currently unavailable</p>
                  </div>
                </div>
              ) : (
            ...
            <div className="pt-6 flex items-center gap-8">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${isOutOfStock ? "bg-red-500" : isLowStock ? "bg-amber-500 animate-pulse" : "bg-green-500 animate-pulse"}`} />
                <span className="text-xs font-bold text-gray-500">
                  {isOutOfStock ? t("out_of_stock") : isLowStock ? `Low Stock (${product.stock} left)` : "In Stock"}
                </span>
              </div>
              <div className="flex items-center gap-2">
            ...
            {/* WhatsApp Order */}
            <a href={`https://wa.me/917987805929?text=Hi! I want to order ${product.name} (${quantity} qty) for ₹${(price * quantity).toLocaleString()}`}
