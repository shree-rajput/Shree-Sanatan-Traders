...
import { LuPackage, LuChevronRight, LuCalendar, LuBox, LuChevronLeft, LuRotateCcw, LuCircleX, LuTruck, LuMapPin } from "react-icons/lu";
    ...
    <div className="min-h-screen bg-gray-50/50 py-12 px-4 md:px-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-10 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">{t("my_orders")} <span className="text-green-600">📦</span></h1>
            <p className="text-sm text-gray-400 mt-1 font-medium">{orders.length} orders</p>
          </div>
          <Link to="/products" className="text-sm font-bold text-gray-400 hover:text-green-600 flex items-center gap-1">
        ...
        {orders.length === 0 ? (
          <div className="bg-white rounded-3xl p-20 text-center border border-gray-100 shadow-sm">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">{t("no_orders")}</h2>
            <p className="text-gray-400 mb-8">{t("no_orders_desc")}</p>
            <Link to="/products" className="inline-flex items-center gap-2 px-8 py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 transition-all shadow-lg shadow-green-100">
            ...
            {orders.map((order) => {
              ...
              return (
                <div key={order._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                  <div className="bg-gray-50/70 px-6 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex gap-6 flex-wrap">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-mono font-bold text-gray-600">#{order._id?.slice(-8).toUpperCase()}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">Payment</p>
                        <p className="text-xs font-bold text-gray-600">{order.paymentMethod === "cod" ? "💵 COD" : order.paymentMethod || "COD"}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${cfg.color}`}>
                  ...
                  <div className="p-6">
                        ...
                        <div key={idx} className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center border border-gray-100 overflow-hidden">
                              {item.image ? <img src={item.image} alt="" className="w-full h-full object-contain" /> : <LuPackage size={18} className="text-gray-300" />}
                            </div>
                            <div>
                          ...
                          <p className="text-sm font-bold text-gray-700">₹{((item.price || 0) * item.quantity).toLocaleString()}</p>
                        </div>
                      ))}
                      {order.items?.length > 3 && <p className="text-xs text-gray-400 font-medium">+{order.items.length - 3} more items</p>}
                    </div>
                    ...
                    {order.trackingId && (
                      <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-center gap-2 mb-4">
                        ...
                        <p className="text-sm text-blue-700 font-bold">Tracking: {order.trackingId}</p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-3 pt-4 border-t border-gray-50 flex-wrap">
                      <Link to={`/orders/${order._id}`} className="flex items-center gap-1 text-xs font-bold text-green-600 border border-green-200 px-4 py-2 rounded-xl hover:bg-green-50 transition-all">
